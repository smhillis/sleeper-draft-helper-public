...
function photoUrl(name) {
  const p = state.players[norm(name)];
  return p ? `https://sleepercdn.com/content/nfl/players/${p.id}.jpg` : '';
}

function rosterForUser() {
  const id = state.user?.user_id;
  return state.rosters.find((r) => r.owner_id === id || (r.co_owners || []).includes(id)) || null;
}
...
function resolveSlot() {
  ...
  if (roster && d.slot_to_roster_id) {
    for (const [s, rid] of Object.entries(d.slot_to_roster_id)) {
      if (Number(rid) === Number(roster.roster_id)) return Number(s);
    }
  }
  return null;
}
...
function mine() {
  return state.picks.filter((p) => Number(p.draft_slot) === Number(state.slot));
}
...
function rosterProfile() {
  ...
  const flex = slots.filter((slot) => ['FLEX', 'REC_FLEX', 'WRRB_FLEX', 'WRRBTE_FLEX'].includes(String(slot).toUpperCase())).length;
  const superflex = slots.filter((slot) => ['SUPER_FLEX', 'QB_FLEX'].includes(String(slot).toUpperCase())).length;
  return {
    QB: count('QB'),
    RB: count('RB'),
    WR: count('WR'),
...
function projectionFor(player) {
  return state.projections[norm(player?.name)] || null;
}
...
function thresholdGames(total, threshold, sd) {
  if (!total) return 0;
...
function statValue(p, key) {
  ...
  if (key === 'pass_fd') return num(p.passCmp) * 0.52;
  if (key === 'rush_fd') return num(p.rushAtt) * 0.24 + num(p.rushTd) * 0.6;
  if (key === 'rec_fd') return num(p.rec) * 0.55;
  if (key === 'pass_2pt') return num(p.passTd) * 0.025;
  if (key === 'rush_2pt') return num(p.rushTd) * 0.025;
  if (key === 'rec_2pt') return num(p.recTd) * 0.025;
  if (key === 'pass_sack') return p.pos === 'QB' ? num(p.passAtt) * 0.06 : 0;
  if (key === 'bonus_pass_yd_300') return thresholdGames(p.passYds, 300, 70);
  if (key === 'bonus_pass_yd_400') return thresholdGames(p.passYds, 400, 70);
  if (key === 'bonus_rush_yd_100') return thresholdGames(p.rushYds, 100, 38);
  if (key === 'bonus_rush_yd_200') return thresholdGames(p.rushYds, 200, 38);
  if (key === 'bonus_rec_yd_100') return thresholdGames(p.recYds, 100, 42);
  if (key === 'bonus_rec_yd_200') return thresholdGames(p.recYds, 200, 42);
  if (key === 'bonus_pass_td_40') return num(p.passTd) * 0.22;
  if (key === 'bonus_pass_td_50') return num(p.passTd) * 0.12;
  if (key === 'bonus_rush_td_40') return num(p.rushTd) * 0.18;
  if (key === 'bonus_rush_td_50') return num(p.rushTd) * 0.10;
  if (key === 'bonus_rec_td_40') return num(p.recTd) * 0.20;
  if (key === 'bonus_rec_td_50') return num(p.recTd) * 0.11;
  if (key === 'bonus_pass_yd_40') return num(p.passYds) / 400;
  if (key === 'bonus_pass_yd_50') return num(p.passYds) / 700;
  if (key === 'bonus_rush_yd_40') return num(p.rushYds) / 300;
  if (key === 'bonus_rush_yd_50') return num(p.rushYds) / 500;
  if (key === 'bonus_rec_yd_40') return num(p.recYds) / 250;
  if (key === 'bonus_rec_yd_50') return num(p.recYds) / 425;
  return 0;
}

const MODELED_KEYS = new Set([
  'pass_att','pass_cmp','pass_yd','pass_td','pass_int','pass_inc','pass_sack','pass_fd','pass_2pt',
  ...
  'bonus_pass_td_40','bonus_pass_td_50','bonus_rush_td_40','bonus_rush_td_50','bonus_rec_td_40','bonus_rec_td_50',
  'bonus_pass_yd_40','bonus_pass_yd_50','bonus_rush_yd_40','bonus_rush_yd_50','bonus_rec_yd_40','bonus_rec_yd_50',
]);

function isOffensiveScoringKey(key) {
  return /^(pass|rush|rec|fum|bonus_pass|bonus_rush|bonus_rec)/.test(key);
}
...
function leaguePoints(p) {
  ...
  Object.entries(s).forEach(([key, raw]) => {
    ...
    if (MODELED_KEYS.has(key)) {
      points += statValue(p, key) * value;
      matched += 1;
    }
  });

  if (p.pos === 'TE' && num(s.bonus_rec_te)) {
    points += num(p.rec) * num(s.bonus_rec_te);
    matched += 1;
  }
  if (p.pos === 'WR' && num(s.bonus_rec_wr)) {
    points += num(p.rec) * num(s.bonus_rec_wr);
    matched += 1;
  }
  if (p.pos === 'RB' && num(s.bonus_rec_rb)) {
    points += num(p.rec) * num(s.bonus_rec_rb);
    matched += 1;
  }
...
function scoringCoverage() {
  ...
  const modeled = active.filter(([key]) => MODELED_KEYS.has(key) || ['bonus_rec_te','bonus_rec_wr','bonus_rec_rb'].includes(key));
...
function scoringSummary() {
  ...
  if (profile.QB >= 2) parts.push(`${profile.QB}QB`);
  else if (profile.SUPER_FLEX) parts.push('Superflex');
...
function render() {
  ...
  $('pickStatus').textContent = state.draft?.status === 'drafting'
    ? `Pick ${state.picks.length + 1} is on the clock`
    : state.draft?.status === 'complete' ? 'Draft complete' : 'Draft not started';
  ...
  const toggle = $('showMoreRecs');
  if (toggle) toggle.onclick = () => { state.showMoreRecommendations = !state.showMoreRecommendations; render(); };

  const np = nextMine();
  $('turnCombo').innerHTML = np.length === 2 && np[1].overall === np[0].overall + 1 && rs[1]
    ? `<b>BACK-TO-BACK PICKS</b><br><strong>Take ${rs[0].name} + ${rs[1].name}</strong>` : '';
}
...
async function connect(username, leagueId) {
  ...
  state.showMoreRecommendations = false;
  try {
    await Promise.all([loadBoard(), loadPlayers()]);
    if (!state.board.length) throw new Error('Rankings are temporarily unavailable.');
    const [user, league, drafts, rosters] = await Promise.all([
      j(`${API}/user/${encodeURIComponent(username)}`),
      j(`${API}/league/${leagueId}`),
      j(`${API}/league/${leagueId}/drafts`),
    ...
    state.slot = resolveSlot();
    if (!state.slot) throw new Error('Sleeper has not assigned a draft slot to this roster yet.');
    state.picks = await j(`${API}/draft/${state.draft.draft_id}/picks`) || [];
    const q = new URLSearchParams({ user: state.username, league: state.leagueId });
    history.replaceState(null, '', `${location.pathname}?${q}`);
    localStorage.setItem('wtdn-user', state.username);
    localStorage.setItem('wtdn-league', state.leagueId);
    ...
    showErr('setupError', e.message || 'Could not connect to Sleeper.');
  } finally {
    $('connectBtn').textContent = 'Show Me Who to Draft Next';
...
async function sync() {
  try {
    ...
    const [league, drafts, rosters] = await Promise.all([
      j(`${API}/league/${state.leagueId}`),
      j(`${API}/league/${state.leagueId}/drafts`),
      j(`${API}/league/${state.leagueId}/rosters`),
    ]);
    state.league = league;
    state.rosters = rosters || [];
    state.draft = (drafts || []).find((d) => String(d.season) === '2026') || (drafts || [])[0] || state.draft;
    state.slot = resolveSlot() || state.slot;
    ...
    showErr('syncError', 'Sleeper data is temporarily unavailable. Retrying automatically.');
    $('lastSync').textContent = 'Retrying…';
  }
}

$('setupForm').addEventListener('submit', (e) => {
  e.preventDefault();
  connect($('username').value.trim(), $('leagueId').value.trim());
});

$('changeLeague').onclick = () => {
  if (state.timer) clearInterval(state.timer);
  state.timer = null;
  state.showMoreRecommendations = false;
...
$('shareBtn').onclick = async () => {
  ...
  try {
    if (navigator.share) await navigator.share({ title: 'Who To Draft Next', url });
...
$('username').value = u;
$('leagueId').value = l;
if (params.get('user') && params.get('league')) connect(params.get('user'), params.get('league'));
